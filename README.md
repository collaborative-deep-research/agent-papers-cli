...
`<ref>` accepts: `2302.13971`, `arxiv.org/abs/2302.13971`, `arxiv.org/pdf/2302.13971`, a **DOI** like `10.1038/s41586-023-06845-4` or `https://doi.org/10.1038/s41586-023-06845-4`, or a **local PDF path** like `./paper.pdf`

Arxiv papers are downloaded once and cached in `~/.papers/`. DOI papers are resolved to open-access PDFs via the bioRxiv API (for `10.1101/...` DOIs) or the [Unpaywall API](https://unpaywall.org/products/api), then cached in `~/.papers/doi-{doi}/`. Local PDFs are read directly from disk — each gets a unique cache directory based on its absolute path (`{stem}-{hash8}`), so two different `paper.pdf` files in different directories won't collide. If you modify a local PDF after it's been parsed, the stale cache is automatically detected and re-parsed.
...
1. **Fetch**: Downloads the PDF from arxiv, resolves a DOI to an open-access PDF (via bioRxiv API or Unpaywall), or reads a local PDF directly — all cached in `~/.papers/<id>/`
